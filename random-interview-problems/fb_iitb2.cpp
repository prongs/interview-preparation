...
struct Node
{
	...
	int parent;
	bool visiting;bool schoolReachable; bool alreadyVisited;
	Node(){
		visiting=schoolReachable=alreadyVisited=false;nextNodes=vector<int>();
	}
};
inline ostream& operator<<(ostream& os, const Node n){return os<<"Node("<<"visiting= "<<n.visiting<<" schoolReachable="<<n.schoolReachable<<" alreadyVisited="<<n.alreadyVisited<<")";}
...
{
	debug(n,nodes[n]);
	if(nodes[n].alreadyVisited)
		return nodes[n].schoolReachable;
	if(nodes[n].visiting)
	{
		// nodes[n].visiting=false;
		// return false;		
		return nodes[n].schoolReachable;
	}
	if(n==school)
	{
		...
		nodes[school].visiting=true;
		nodes[school].alreadyVisited=true;
		numreachables++;
		return true;
	}
	bool reachable=nodes[n].schoolReachable;
	bool av = true;	
	nodes[n].visiting=true; //have to do it here to avoid infinite loop.
	for (int i = 0; i < nodes[n].nextNodes.size(); ++i)
	{
		reachable |= dfs(nodes[n].nextNodes[i]);
		nodes[n].schoolReachable = reachable;
		av &= nodes[nodes[n].nextNodes[i]].alreadyVisited;
	}
	nodes[n].alreadyVisited=av;
	nodes[n].visiting=false;
	if(reachable)
	{
