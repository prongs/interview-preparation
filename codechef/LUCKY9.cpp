...
char n[5010];/*integers to be scanned every loop*/
...
int main(int argc, char const *argv[])
{
	...
	while(T--)
	{
		...
		ss(n);
		int len = strlen(n);
		vector<int> fourBefore(len);
		vector<int> sevenAfter(len);
		fourBefore[0] = 0;
		sevenAfter[len-1] = 0;
		for (int i = 1; i < len; ++i)
		{
			fourBefore[i] = fourBefore[i-1]+(1?n[i-1]=='4':0);
		}
		for (int i = len-2; i >= 0; --i)
		{
			sevenAfter[i] = sevenAfter[i+1]+(1?n[i+1]=='7':0);
		}
	}
